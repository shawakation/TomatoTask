...
public class ChangeSqlite {
	...
	public ArrayList<Notepad> query(SQLiteDatabase paramSQLiteDatabase) {
		...
		Cursor localCursor = paramSQLiteDatabase.query(table, new String[] {"id", "title", "content", "date"}, null, null, null, null,null);
		while (true) {
			...
			localNotepad.setid(localCursor.getString(localCursor.getColumnIndex("id")));
			localNotepad.setTitle(localCursor.getString(localCursor.getColumnIndex("title")));
			localNotepad.setContent(localCursor.getString(localCursor.getColumnIndex("content")));
			localNotepad.setdata(localCursor.getString(localCursor.getColumnIndex("date")));
	...
	public void update(SQLiteDatabase paramSQLiteDatabase, Notepad paramNotepad) {
		...
		localContentValues.put("content", paramNotepad.getContent());
		localContentValues.put("date", paramNotepad.getdata());
		paramSQLiteDatabase.update(table, localContentValues, "id=" + paramNotepad.getid(), null);
		paramSQLiteDatabase.close();
	}
}
